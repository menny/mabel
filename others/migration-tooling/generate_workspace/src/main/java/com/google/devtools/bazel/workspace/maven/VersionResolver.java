...
class VersionResolver {
    ...
    String resolveVersion(String groupId, String artifactId, String classifier, String versionSpec)
        ...
        if (isInvalidRangeResult(versions)) {
            if (!isVersionRange(versionSpec)) {
                versions = asList(versionSpec);
            } else {
                String errorMessage =
                    messageForInvalidArtifact(groupId, artifactId, versionSpec, classifier, "Invalid Range Result");
                throw new InvalidArtifactCoordinateException(errorMessage);
            }
        }
    ...
    private List<String> requestVersionList(String groupId, String artifactId, String classifier, String versionSpec)
        ...
        Artifact artifact = ArtifactBuilder.fromCoords(groupId, artifactId, classifier, transformedSpec);
    ...
    private String selectVersion(String versionSpec, List<String> versions) {
        ...
        //this will return the spec with the classifier, transforming a bit to make it usable.
        //removing the classifier and removing the version range spec
        return versions.get(index).replaceAll("^.*\\[", "").replaceAll(",.*[)\\]]$", "");
    }
