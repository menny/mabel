...
public class WritersTests {
  ...
  private static final String TRANSITIVE_TARGETS_MACRO_OUTPUT =
      "\n"
          ...
          + "\n"
          + "    This will be used in cases when some dependencies require aar_import rule"
          + " implementation.\n"
          + "\n"
          + "    Args:\n"
          ...
          + "\n"
          + "    fail(\n"
          + "        \"Unable to create target {} since it is a aar_import which was not provided."
          + " Add argument aar_import when calling macro_name.\"\n"
          + "            .format(name),\n"
          + "    )\n"
          + "\n"
          + "def macro_name(\n"
          ...
          + "\n"
          + "    You can provide your own implementation of jvm_import, aar_import, etc. This can"
          + " be used\n"
          + "    in cases where you need to shade (or jar_jar or jetify) your jars.\n"
          + "\n"
          + "    Args:\n"
          + "        name: a unique name for this macro. Not needed to specify.\n"
          + "        java_library: rule implementation for java_library. Defaults to"
          + " native.java_library.\n"
          + "        _java_plugin: rule implementation for java_plugin. Defaults to"
          + " native.java_plugin.\n"
          + "        _jvm_import: rule implementation for jvm_import. Defaults to jvm_import.\n"
          + "        _aar_import: rule implementation for aar_import. Required only if you have"
  ...
  public void testRepositoryRulesMacroWriterWithSources() throws Exception {
    final List<Target> targets =
        new TargetsBuilders.HttpTargetsBuilder(false, dep -> URI.create(""))
            .buildTargets(
                Dependency.builder()
                    .mavenCoordinate(
                        MavenCoordinate.create("net.evendanan", "dep1", "1.2.3", "jar"))
    ...
    RuleWriters.HttpRepoRulesMacroWriter writer =
        new RuleWriters.HttpRepoRulesMacroWriter(
            outputFile, "macro_name", "none_default_name_mabel");
  ...
  public void testTransitiveRulesAliasWriterMultiplePublicTargets() throws Exception {
    ...
    writer.write(
        Arrays.asList(
            new Target("net.evendanan.dep2:artifact:2.0", "rule", "name_name_2")
                .setPublicVisibility(),
            new Target("net.evendanan.dep2:artifact:2.0", "rule", "name_name_2_somethingelse")
                .setPublicVisibility()));
    Assert.assertEquals(
  ...
  public void testGetFilePathFromUrl() throws Exception {
    Assert.assertEquals(
        "net/evendanan/lib1/", RuleWriters.getFilePathFromMavenName("net.evendanan", "lib1"));
    Assert.assertEquals(
        "net/even-danan/lib1/", RuleWriters.getFilePathFromMavenName("net.even-danan", "lib1"));
    Assert.assertEquals(
  ...
  public void testTransitiveRulesMacroWriter() throws Exception {
    ...
    Target targetWithComment =
        new Target("net.evendanan.dep1:artifact:1.2.3", "java_library", "name_name_1");
    targetWithComment.addComment("comment one");
    targetWithComment.addComment("comment two");
    Target targetWithoutComment =
        new Target("net.evendanan.dep2:artifact:2.0", "rule", "name_name_2");
