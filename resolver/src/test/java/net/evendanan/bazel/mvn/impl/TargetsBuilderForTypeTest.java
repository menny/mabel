...
public class TargetsBuilderForTypeTest {
  ...
  public void generateBuilderForTheRightType() {
    ...
    TargetsBuilderForType underTest =
        new TargetsBuilderForType(mvn -> returnValue.get(), dep -> null);
    ...
    Assert.assertSame(
        TargetsBuilders.AAR_IMPORT_WITHOUT_EXPORTS, underTest.generateBuilder(dependency));

    returnValue.set(TargetType.naive);
    Assert.assertTrue(
        underTest.generateBuilder(dependency) instanceof TargetsBuilderForType.NaiveBuilder);

    returnValue.set(TargetType.processor);
    Assert.assertTrue(
        underTest.generateBuilder(dependency) instanceof TargetsBuilderForType.ProcessorBuilder);

    returnValue.set(TargetType.auto);
    Assert.assertTrue(
        underTest.generateBuilder(dependency) instanceof TargetsBuilderForType.AutoBuilder);
  }
  ...
  public void testThrowsExceptionOnUnhandledType() {
    TargetsBuilderForType underTest =
        new TargetsBuilderForType(mvn -> TargetType.valueOf("unknown"), dep -> null);
    underTest.generateBuilder(dependency);
  }
}
