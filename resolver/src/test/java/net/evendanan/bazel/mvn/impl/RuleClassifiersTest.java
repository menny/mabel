...
public class RuleClassifiersTest {
  ...
  public void testAarClassifier() {
    Dependency dep =
        Dependency.builder().mavenCoordinate(MavenCoordinate.create("", "", "", "aar")).build();
    Assert.assertSame(
  ...
  public void testPomClassifier() {
    Dependency dep =
        Dependency.builder().mavenCoordinate(MavenCoordinate.create("", "", "", "pom")).build();
    Assert.assertSame(
        TargetsBuilders.POM_IMPORT, new RuleClassifiers.PomClassifier().classifyRule(dep).get(0));
  ...
  public void testJarInspector_java_plugin() throws Exception {
    ...
    final Function<Dependency, URI> dependencyURIFunction =
        dep -> {
          try {
            return Resources.getResource("dataenum-processor-1.0.2.jar").toURI();
          } catch (URISyntaxException e) {
            throw new RuntimeException(e);
          }
        };
    final TargetsBuilder processorFormatter =
        new RuleClassifiers.JarInspector(dependencyURIFunction)
            .findAllPossibleBuilders(dependency).stream()
                .filter(
                    possibleBuilder ->
                        possibleBuilder instanceof TargetsBuilders.JavaPluginFormatter)
                .findFirst()
                .orElse(null);
  ...
  public void testJarInspector_java_plugin_with_comments() throws Exception {
    ...
    final Function<Dependency, URI> dependencyURIFunction =
        dep -> {
          try {
            return Resources.getResource("dataenum-processor-1.0.2-with-comments.jar").toURI();
          } catch (URISyntaxException e) {
            throw new RuntimeException(e);
          }
        };
    final TargetsBuilder processorFormatter =
        new RuleClassifiers.JarInspector(dependencyURIFunction)
            .findAllPossibleBuilders(dependency).stream()
                .filter(
                    targetsBuilder -> targetsBuilder instanceof TargetsBuilders.JavaPluginFormatter)
                .findFirst()
                .orElse(null);
    Assert.assertNotNull(processorFormatter);
    List<String> processorClasses =
  ...
  public void testPriority() {
    ...
    TargetsBuilders.CompositeBuilder compositeBuilder =
        (TargetsBuilders.CompositeBuilder) actualTargetBuilder;
  ...
  public void testJarClassifierNoResults() {
    RuleClassifier classifier =
        new RuleClassifiers.JarClassifier(dependency -> Collections.emptyList());
  ...
  public void testJarClassifierUnknownResults() {
    RuleClassifier classifier =
        new RuleClassifiers.JarClassifier(
            dependency -> Collections.singletonList(TargetsBuilders.AAR_IMPORT));
