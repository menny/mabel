...
public class SerializationTest {
  ...
  public void testHappyPath() {
        ...
        Resolution.create(
            ...
            Arrays.asList(
                ...
                Dependency.builder()
                    ...
                    .licenses(
                        Collections.singletonList(License.create("Apache-2", "http://example.com")))
                    .dependencies(
                        Arrays.asList(
    ...
    final ResolutionOutput resolutionOutput =
        ResolutionOutput.create(TargetType.auto, ExportsGenerationType.inherit, false, resolution);
    ...
    final ResolutionOutput actualOutput =
        serialization.deserialize(new StringReader(builder.toString()));
    Assert.assertEquals(resolutionOutput.targetType(), actualOutput.targetType());
    Assert.assertEquals(
        resolutionOutput.exportsGenerationType(), actualOutput.exportsGenerationType());
    ...
    Assert.assertEquals(
        resolution.allResolvedDependencies().size(), actual.allResolvedDependencies().size());
    ...
    for (int depIndex = 0; depIndex < expectedDependencies.size(); depIndex++) {
      ...
      Assert.assertArrayEquals(
          expectedDep.runtimeDependencies().toArray(), actualDep.runtimeDependencies().toArray());
    }
  }
}
