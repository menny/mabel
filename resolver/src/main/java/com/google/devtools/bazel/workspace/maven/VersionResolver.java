...
public class VersionResolver {
    ...
    private static String messageForInvalidArtifact(
            String groupId, String artifactId, String versionSpec, String classifier, String errorMessage) {
        return String.format("Unable to find a version for %s:%s:%s (with classifier '%s') due to %s",
    ...
    String resolveVersion(String groupId, String artifactId, String classifier, String versionSpec)
        ...
        if (VERSION_SPEC_CONTAINER.matcher(versionSpec).matches()) {
            ...
            try {
                ...
                String errorMessage =
                        messageForInvalidArtifact(groupId, artifactId, versionSpec, classifier, e.getMessage());
            ...
            if (versions==null || versions.isEmpty()) {
                ...
                if (versionItemMatcher.find()) {
                    ...
                    if (debugLogs) System.out.println("Failed to resolve version for spec " + versionSpec + ". Returning " + defaultVersion);
                    return defaultVersion;
                } else {
                    if (debugLogs) System.out.println("Failed to resolve version for spec " + versionSpec + " and could not identify range. Returning " + versionSpec);
                    return versionSpec;
                }
            }

            if (debugLogs) System.out.println("Found valid versions for spec " + versionSpec + ": " + String.join(",", versions));
     ...
     */
    private List<String> requestVersionList(String groupId, String artifactId, String classifier, String versionSpec)
            throws VersionRangeResolutionException {
        Artifact artifact = ArtifactBuilder.fromCoords(groupId, artifactId, classifier, versionSpec);
