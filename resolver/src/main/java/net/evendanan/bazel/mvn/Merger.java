...
public class Merger {
  ...
  private Merger(final CommandLineOptions options) throws IOException {
    ...
    repositoryRulesMacroWriter =
        new RuleWriters.HttpRepoRulesMacroWriter(
            macrosFile, "generate_workspace_rules", options.repository_rule_name);
    targetsMacroWriter =
  ...
  public static void main(String[] args) throws Exception {
    ...
    Collection<Resolution> resolutions =
        resolutionOutputs.stream().map(ResolutionOutput::resolution).collect(Collectors.toList());
    ...
    driver.writeResults(
        resolutionOutputs, dependencies, testOnlyDeps, downloader, options, dependencyTools);
    ...
    if (!options.output_pretty_dep_graph_filename.isEmpty()) {
      ...
      try (final OutputStreamWriter fileWriter =
          new OutputStreamWriter(new FileOutputStream(prettyOutput, false), Charsets.UTF_8)) {
  ...
  private void writeResults(
      ...
      throws Exception {
    final Set<MavenCoordinate> rootDependencies =
        resolutions.stream().map(r -> r.resolution().rootDependency()).collect(Collectors.toSet());
        ...
        resolutions.stream()
            ...
            .map(
                p ->
                    Pair.of(
                        p.getLeft(),
                        p.getRight().equals(ExportsGenerationType.inherit)
                            ? options.exports_generation
                            : p.getRight()))
            .collect(
                Collectors.toMap(
                    Pair::getKey, Pair::getValue, ExportsGenerationType::prioritizeType))
            .entrySet()
            .stream()
            .filter(
                p -> {
    ...
    if (!depsFolder.isDirectory() && !depsFolder.mkdirs()) {
      throw new IOException(
          "Failed to create folder for dependency files: " + depsFolder.getAbsolutePath());
    }

    try (final OutputStreamWriter fileWriter =
        new OutputStreamWriter(
            new FileOutputStream(new File(depsFolder, "BUILD.bazel"), false), Charsets.UTF_8)) {
    ...
    final Map<MavenCoordinate, TargetType> targetTypeMap =
        resolutions.stream()
            .collect(
                Collectors.toMap(
                    r -> r.resolution().rootDependency(),
                    r ->
                        r.targetType().equals(TargetType.inherit) ? options.type : r.targetType()));
    final Function<Dependency, TargetsBuilder> ruleMapper =
        buildRuleMapper(
            downloader,
            dep ->
                targetTypeMap.getOrDefault(
                    dep,
                    TargetType
                        .auto /*once we move to jvm_import, this should be changed to naive*/),
            rootDependencies,
            resolvedDependencies);
