...
class Adapters {
  ...
  private static final Type LICENSES_LIST_TYPE = new TypeToken<Collection<License>>() {}.getType();
  ...
  static class ResolutionOutputSerializer implements JsonSerializer<ResolutionOutput> {
    ...
    public JsonElement serialize(
      ...
      jsonObject.add("t", context.serialize(resolution.targetType(), TargetType.class));
      jsonObject.add(
          "e", context.serialize(resolution.exportsGenerationType(), ExportsGenerationType.class));
      jsonObject.add("to", context.serialize(resolution.testOnly(), boolean.class));
      jsonObject.add("r", context.serialize(resolution.resolution(), Resolution.class));
  ...
  static class ResolutionSerializer implements JsonSerializer<Resolution> {
    ...
    @Override
    public JsonElement serialize(
        final Resolution resolution, final Type type, final JsonSerializationContext context) {
      JsonObject jsonObject = new JsonObject();

      jsonObject.add("c", context.serialize(resolution.rootDependency(), MavenCoordinate.class));
      jsonObject.add(
          "d", context.serialize(resolution.allResolvedDependencies(), DEPENDENCIES_LIST_TYPE));
  ...
  static class DependencySerializer implements JsonSerializer<Dependency> {
    ...
    @Override
    public JsonElement serialize(
        final Dependency dependency, final Type type, final JsonSerializationContext context) {
      JsonObject jsonObject = new JsonObject();
