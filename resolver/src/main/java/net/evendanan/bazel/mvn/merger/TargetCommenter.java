...
public class TargetCommenter {
  ...
  public TargetCommenter(
      Set<MavenCoordinate> rootCoordinates, Collection<Dependency> resolvedDependencies) {
    this.rootCoordinatesAsString =
        rootCoordinates.stream()
    ...
    resolvedDependencies.forEach(
        dependency -> {
          ...
          collectReverseDependencies(
              requesterMavenCoordinate, dependency.dependencies(), reverseDependenciesSet);
          collectReverseDependencies(
              requesterMavenCoordinate, dependency.exports(), reverseDependenciesSet);
          collectReverseDependencies(
              requesterMavenCoordinate, dependency.runtimeDependencies(), reverseDependenciesSet);
        });

    reverseDependencies =
        reverseDependenciesSet.entrySet().stream()
  ...
  private static void collectReverseDependencies(
      String ownerKey,
      Collection<MavenCoordinate> deps,
      Map<String, Set<String>> reverseDependencies) {
    deps.forEach(
        dep ->
            reverseDependencies.compute(
                dep.toMavenString(),
  ...
  private void addRequiredByComment(Target target) {
    ...
    if (requesters != null) {
      requesters.forEach(
          requester ->
              target.addComment(
                  String.format(Locale.ROOT, "This is a dependency of '%s'.", requester)));
    }
  }
