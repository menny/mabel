...
public class CompositeFormatterTest {
    ...
    public void testCompositeFormatter() {
        ...
        final String combinedFormat = compositeFormatter.formatRule(rule);

        Mockito.verify(formatter1).formatRule(Mockito.same(rule));
        Mockito.verifyNoMoreInteractions(formatter1);
        Mockito.verify(formatter2).formatRule(Mockito.same(rule));
